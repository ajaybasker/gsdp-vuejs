<template>
  <div class="min-h-screen bg-slate-50 font-sans">
    ...
    <main class="w-full px-4 md:px-8 xl:px-12 py-16 space-y-20">
      <Reveal>
        <div class="flex flex-col md:flex-row gap-8 items-center bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-8 sm:p-12">
          <div class="flex-1 space-y-4">
            <h2 class="font-serif text-[28px] font-black text-brand-600 tracking-tight">Vision</h2>
            <blockquote class="text-xl sm:text-2xl font-extrabold text-brand-950 leading-snug">
          ...
          <div class="w-full md:w-5/12 shrink-0 rounded-2xl overflow-hidden shadow-md">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSEb2_0QoimfLNxtWfIeCZu2ZyrXhuNwlJE-BoVO4p5Gw&s=10" alt="Salesian Vision" class="w-full h-full object-cover" />
          </div>
        </div>
      </Reveal>

      <Reveal :delay="100">
        <div class="flex flex-col md:flex-row-reverse gap-8 items-center bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-8 sm:p-12">
          <div class="flex-1 space-y-4">
            <h2 class="font-serif text-[28px] font-black text-brand-600 tracking-tight">Mission</h2>
            <blockquote class="text-xl sm:text-2xl font-extrabold text-brand-950 leading-relaxed">
          ...
          <div class="w-full md:w-5/12 shrink-0 rounded-2xl overflow-hidden shadow-md">
            <img src="https://www.donbosco.press/wp-content/uploads/2023/09/154-Spedizione-Missionaria-0_tn.jpg" alt="Salesian Mission" class="w-full h-full object-cover" />
          </div>
        </div>
      </Reveal>
        ...
        <div class="bg-brand-950 text-white rounded-3xl shadow-2xl overflow-hidden">
          <div class="p-8 sm:p-12 lg:p-16 grid lg:grid-cols-12 gap-12 items-center relative">
            ...
            <div class="space-y-6 relative z-10 lg:col-span-7 xl:col-span-8">
              ...
              <p class="text-base sm:text-lg leading-relaxed text-slate-100">
              ...
              <blockquote class="border-l-4 border-amber-500 pl-5 py-2 text-xl font-bold italic text-amber-400">
                "Da mihi animas, cetera tolle"<br />
                <span class="text-base text-amber-200/80 not-italic mt-1 block">"Give me souls, take away the rest."</span>
              </blockquote>
            </div>
            <div class="relative z-10 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 lg:col-span-5 xl:col-span-4 max-w-md mx-auto w-full">
              <img src="https://salesiansisterswest.org/sites/salesian/files/uploads/images/new_st._fracis_de_sales.jpg" alt="Salesian History" class="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </Reveal>
      ...
      <Reveal>
        <div>
          <div class="mb-10 text-center max-w-2xl mx-auto">
            <h2 class="text-sm font-bold tracking-widest text-brand-600 uppercase mb-3">Leadership</h2>
            <h3 class="text-[28px] font-extrabold text-brand-950">The General Council</h3>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(member, idx) in GENERAL_COUNCIL" :key="member.name" class="group flex flex-col items-center text-center rounded-3xl bg-white border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              <div :class="`h-1.5 w-full bg-gradient-to-r ${accentFor(idx)}`" />
              <div class="p-8 flex flex-col items-center">
                <div class="h-36 w-36 rounded-full overflow-hidden mb-5 shadow-inner ring-4 ring-slate-50 group-hover:ring-brand-50 transition-all duration-300">
                  <img :src="member.photo" :alt="member.name" class="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <h4 class="text-[16px] font-extrabold text-brand-950 mb-1.5">{{ member.name }}</h4>
                <p class="text-[11px] font-bold text-brand-500 uppercase tracking-widest mb-4">{{ member.role }}</p>
                <p class="text-[13px] leading-relaxed text-slate-500">{{ member.desc }}</p>
              </div>
            </div>
          </div>
        </div>
